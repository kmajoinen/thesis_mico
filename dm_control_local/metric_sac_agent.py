...
import optax
from atari import metric_utils
...
def train(network_def,
  ...
  def loss_fn(
    ...
    def prev_latent(state, action):
      return network_def.apply(old_params, state, action,
                               method=network_def.critic)
    ...
    # Trust loss.
    _, _, representations_old = jax.vmap(
        prev_latent)(states, sampled_actions)
    representations_old = jnp.squeeze(representations_old)
    # trust_loss = jnp.mean(optax.huber_loss(critic_z, critic_z_old, delta=1.0))
    # trust_loss = jnp.mean(losses.huber_loss(representations, representations_old))
    # trust_loss = jnp.mean(jax.vmap(losses.huber_loss)(representation, representation_old))
    # trust_loss = jnp.mean(losses.huber_loss(representations, representations_old))
    trust_loss = jnp.mean(losses.huber_loss(representations, representations_old, delta=1.0))
    ...
    combined_loss = (1. - mico_weight) * sac_loss + mico_weight * mico_loss + trust_beta*trust_loss
  ...
  returns = {
      ...
      'Losses/Trust': aux_vars['trust_loss'],
...
class MetricSACAgent(sac_agent.SACAgent):
  ...
  def __init__(self,
    ...
    self.old_params = self.network_params
    self._trust_beta = trust_beta
    ...
    if self._replay.add_count > self.min_replay_history:
      if self.training_steps % self.update_period == 0:
        ...
        train_returns = train(
            ...
            self.old_params,
            self._trust_beta)
        temp_params = train_returns['network_params']
        self.old_params = self.network_params
        self.network_params = temp_params
